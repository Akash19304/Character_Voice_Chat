...
class AudioManager:
    ...
    def __init__(self):
        ...
        # Use higher buffer because it increases the internal buffer size to help avoid audio glitches.
    ...
    def play_audio(self, file_path, sleep_during_playback=True, delete_file=False, play_using_music=True):
        ...
        if not pygame.mixer.get_init(): # Reinitialize mixer if it's not already initialized
            pygame.mixer.init(frequency=48000, buffer=1024) 
        if play_using_music:
        ...
        if sleep_during_playback:
            ...
            _, extension = os.path.splitext(file_path) # Get the extension of this file
...
#     WAV_FILEPATH = "___Msg3426107777569136159.wav"
